...
1. Download the latest distribution, e.g. [**STLSoft-1.10-1.10.8.zip**](https://github.com/synesissoftware/STLSoft-1.10/archive/refs/tags/1.10.8.zip);
...
$ mkdir -p ~/open-source
$ cd ~/open-source
$ unzip STLSoft-1.10-1.10.8.zip
```

3. Define an environment variable `STLSOFT`, whose value is the directory in which you unzipped it, e.g. `STLSOFT=~/open-source/STLSoft-1.10-1.10.8`, and then specify `$(STLSOFT)/include` (**UNIX**) or `%STLSOFT%\include` (**Windows**) in your project files and makefiles;
...
**/usr/local/stlsoft/stlsoft-1.10.8**
...
**C:\3pty\STLSoft\stlsoft-1.10.8**
then you should set the environment variable `STLSoft` to
**C:\3pty\STLSoft\stlsoft-1.10.8**
in which case the file `stlsoft/stlsoft.h`, for example, will be located in `C:\3pty\STLSoft\stlsoft-1.10.8\stlsoft\stlsoft.h`.
